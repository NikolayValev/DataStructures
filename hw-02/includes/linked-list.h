...
public:
  ...
  node(node<T> *next, node<T> *prev,
       T data = 0) //  : next(next), prev(prev), data(data)
  {
...
public:
  ...
  bool one_element() { return length == 1; }

  bool empty_or_one() {
    if (is_empty() || one_element()) {
      return 1;
    }
    return 0;
  }
  ...
  void SplitAndSwitch() {
    ...
    tail = pos.target;
    pos.target->next->prev = nullptr;
    pos.target->next = nullptr;
  }
  ...
  void Reverse() {
    ...
    tail = pos.target;
    while (pos.valid()) {
      node<T> *temp = pos.target->next;
      pos.target->next = pos.target->prev;
      pos.target->prev = temp;
      pos.back();
    }
  }
  ...
  void insert(iterator<T> pos, T value) {
    ...
    if (pos.target == head) {
      ...
      // Inserting before an element which has an element before it, and after
      // it...
  ...
  void push_front(T value) {
    ...
    if (is_empty()) {
      ...
      // As an exercise, prove that "old_head" IS tail on a list with one
      // element, so tail is going to automatically be kept in sync.
    }
...
private:
  ...
  void debug_printout() {
    ...
    while (current != nullptr) {
      std::cerr << current << ":  " << current->prev << " <= " << current->data
...
public:
  ...
  bool valid() { return target != nullptr; }
  ~iterator<T>() {}
